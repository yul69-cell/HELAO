...
# CV simulator
def cvsim(vard, DM):
    ts, eta = egen(vard['etai'], vard['eta0'], vard['eta1'], vard['etaf'], vard['v'], vard['cyc'], vard['daq'])
    L = len(eta)-1
    F = 96485  #[=] C/mol, Faraday's constant
    R = 8.3145  #[=] J/mol-K, ideal gas constant
    f = F / (R * vard["T"])  #[=] 1/V, normalized Faraday's constant at room temperature
    tk = (np.abs(vard['etai']-vard['eta0']) + np.abs(vard['eta0']-vard['eta1']) + np.abs(vard['eta1']-vard['etaf']))/vard['v'] #[=] s, characteristic exp. time (pg 790). In this case, total time of fwd and rev scans
...
def lssim(vard, DM):
    ts, eta = egen(vard['etai'], vard['etaf'], vard['etaf'], vard['etaf'], vard['v'], 0.5, vard['daq'])
    L = len(eta)-1
    F = 96485  #[=] C/mol, Faraday's constant
    R = 8.3145  #[=] J/mol-K, ideal gas constant
    f = F / (R * vard["T"])  #[=] 1/V, normalized Faraday's constant at room temperature
    tk = (np.abs(vard['etai']-vard['etaf']))/vard['v'] #[=] s, characteristic exp. time (pg 790). In this case, total time of fwd and rev scans
    Dt = tk/L #[=] s, delta time (Eqn B.1.10, pg 790)
    Dx = np.sqrt(vard['D']*Dt/DM) #[=] cm, delta x (Eqn B.1.13, pg 791)
    j = np.int(np.ceil(4.2*L**0.5)+5) #number of boxes (pg 792-793). If L~200, j=65
    ktk = vard['kc']*tk #dimensionless kinetic parameter (Eqn B.3.7, pg 797)
...
class GamryDtaqEvents(object):
    ...
    def simulate(self, sigramp):
        ...
        while time.time() < self.stop_time:
            ...
            self.acquired_points = [[t, v, 0.0, j] for t,v,j in zip(fullt, fullv, fullj) if t<(time.time() - self.start_time)]
